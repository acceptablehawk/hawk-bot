...
from functions import CHAMPKDA
from functions import KDA
from functions import LIVE
...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
        ...
        #        if checking == userid:
         ...
         #           try:
        #                initialbal = ''
        #               for z in range(18,200):
        #                    initialbal += x[z]
                        
       #                 await message.channel.send(initialbal)
                        
        #            except:
       #                 k = 1
                        
         #       elif checking != userid:
         #           fx.write(str(message.author.id) + '0')
                
                
                
        if message.content.lower().replace(' ','').startswith('h!bal'):
            bal = ''
            userid = str(message.author.id)
            checking = ''
            counter = 0
                    ...
                    for y in range(0,18):
                        ...
                        if checking == userid:
                            ...
                            for x in alt:
                                ...
                                try:
                                    for y in range(18, 2000):
                                        bal += x[y]
                                        embed=discord.Embed(title="Account Information", color=0xff0000)
                                        embed.add_field(name='Account Holder: ' + str(message.author), value='Account Balance: ' + 'Ψ' + str(bal), inline=False)
        ...
        if message.content.lower().startswith('h!replay'):
            user = ''
            for x in range(len(message.content)):
                if x > 8:
                    user += message.content[x]
        ...
        #    list = ['/Users/neekpanchal/Downloads/blackmarket/alienloud.jpg', '/Users/neekpanchal/Downloads/blackmarket/bagged_cocaine.jpg', '/Users/neekpanchal/Downloads/blackmarket/bush.jpg', "/Users/neekpanchal/Downloads/blackmarket/chickenuggies.jpeg", '/Users/neekpanchal/Downloads/blackmarket/heroine.jpg']
        #    choice = random.choice(list)
        #    fp = open(choice, 'rb')
        #    channel = client.get_channel(791196807057113090)
        #    await channel.send(file=discord.File(fp, choice))
        #    if choice == list[0]:
        #        await channel.send('Space Weed $ - 1 0 0 0 ')
                
        #    elif choice == list[1]:
        #        await channel.send('Pure Snow $ - 5 0 0')
                
        #    elif choice == list[2]:
        #        await channel.send('Scum Stank Rabbit Weed $ - 0 . 0 5')

        #    elif choice == list[3]:
        #        await channel.send('Pure Nuggets $ - 1 0 . 7 5')
                
        #    elif choice == list[4]:
        #        await channel.send('HEROBRINE - $ - 6 6 6')
        ...
        if message.content.lower().replace(' ', '').startswith('h!keaunpls'):
            user = await client.fetch_user(706390527217500161)
            await user.send('https://tenor.com/view/michael-michael-shirt-michael-shirt-rip-shirt-rip-shirt-rip-sex-gif-18657442')
            
                
        # msg = '{0.author}'.format(message)
        # spam = ["fuckyou", "stop inting", "ur shit kid", "stop feeding", "ur so bad", "AKINTER", "SHITTER", "AKSHITTER", "LEARN TO LEAGUE", 'On foenem grave you dogshit bro']
        # if msg == 'AKBlitz0#0054':
        #    await message.channel.send(random.choice(spam))
